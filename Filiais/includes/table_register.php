...
while (($linha = fgetcsv($file, 100000, ";")) !== false) {
    ...
    if (stripos($cidade, $unidadeServico) !== false) {
        ...
        if ($dataObjC && $dataObjD) {
            // Se a data de chegada não tiver hora, considera início do dia
            if (strlen($dataChegada) === 10) { // formato d/m/Y
                $dataObjC->setTime(0,0,0);
            }

            // Se a data de recebimento não tiver hora, também considera início do dia
...
// Atrasos por empresa - mês atual e mês passado
$atrasosMesAtual = [];
$atrasosMesPassado = [];
...
// Totais semana atual (domingo a sábado)
$hoje = new DateTime();
...
// Amostras por empresa - mês atual e mês passado
$amostrasMesAtual = [];
$amostrasMesPassado = [];
